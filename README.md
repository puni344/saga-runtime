...
- **7-check Invariant Verifier**: noDuplicateDebit, moneyConserved, validTerminalState, stateConsistent, retryWasBlocked, refundIdempotent, oneDebitPerPayment
...
- **255 tests** across runtime / payment-domain / webhook / AI / adversarial / process-restart / contract / scorecard / security-hardening suites (one `node --test tests/*.test.js` run, zero failures)
...
| Integrity proof | `src/verifier.js` 7-check verifier + adversarial benchmark | Proof layer: noDuplicateDebit, moneyConserved, validTerminalState, stateConsistent, retryWasBlocked, refundIdempotent, oneDebitPerPayment. |
...
The Store reads its file once at construction, so a live process cannot see sagas created by another live process after that point; two processes racing the same already-known saga now fail cleanly rather than colliding. The store is backed by a single SQLite database (`data/sagas.db`): every transition commits the saga snapshot **and** the rail payment row inside one `BEGIN IMMEDIATE` transaction, and the database itself is the compare-and-swap. A process whose in-memory snapshot is stale — another process committed since it last saved — is rejected with a `CONCURRENT_ACCESS` error instead of silently last-writer-winning a lost update. SQLite's writer lock means two processes can no longer both pass a check with the exact same starting snapshot; one commits and the other is refused. This is real multi-process serialization at the point of write, but it is **not a distributed lock**: the losing writer is rejected and must reload against the newest state (or be retried), it is not queued, coordinated, or merged.
...
- **`recoveryRate`** — fraction of cases whose `invariantPass` is true, computed by the 7-check verifier over the financial invariants. This is a real, independent measurement. Currently 1.0 with 0 invariant violations.
